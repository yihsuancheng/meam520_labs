...
targets = [
    ...
    transform( np.array([-0.03, .227, .502]),  np.array([pi/6,5/6*pi,7/6*pi]) ),
...
'''
targets = [
    transform( np.array([.3, -0.2, .5]), np.array([-pi/3,pi/2,pi/2])            ),
    #transform( np.array([-0.03, .227, .502]),  np.array([pi/6,5/6*pi,7/6*pi]) ),
    #transform( np.array([.4, 0, .5]),    np.array([0,pi,pi])            ),
    #transform( np.array([.7, 0, .4]),    np.array([0,pi,pi])            ),
    #transform( np.array([.2, .5, 0.5]),  np.array([0,pi,pi])            ),
    #transform( np.array([0.4, -.3, .5]), np.array([0,pi,pi])            ),
    #transform( np.array([-.2, .4, .8]),  np.array([pi/6,5/6*pi,7/6*pi]) ),
    #transform( np.array([.6, 0, .4]),    np.array([0,pi,pi])            ),
    #transform( np.array([.3, .5, .6]),   np.array([pi/4,pi-pi/2,pi])    ),
    #transform( np.array([.2, .6, 0.5]),  np.array([pi/2,pi/2,pi])       ),
]
'''
...
if __name__ == "__main__":
    ...
    for i, target in enumerate(targets):
        ...
        print("initial configuration is ", seed)
        start = perf_counter()
    ...
    success_rate = success_count / total_poses

    print("Mean time is {mean_time} seconds".format(mean_time=mean_time))
    print("Median time is {median_time} seconds".format(median_time=median_time))
    print("Max time is {max_time} seconds".format(max_time=max_time))
    
    print("Mean iteration is {mean_iterations}".format(mean_iterations=mean_iterations))
    print("Median iteration is {median_iterations}".format(median_iterations=median_iterations))
    print("Max iteration is {max_iterations}".format(max_iterations=max_iterations))
